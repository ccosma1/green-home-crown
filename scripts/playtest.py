#!/usr/bin/env python3
"""Static checks plus a 1D fair-player leak sim for L1-70."""
...
def main() -> None:
    ...
    must(HTML.count("{ gap:") >= 420, "L1-70 waves")
    ...
    for li, (name, waves) in enumerate(packed):
        ...
        if li == 69:
            must(leaks <= 1, "L70 finale too leaky: %s" % leaks)
